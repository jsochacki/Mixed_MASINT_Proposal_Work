...
class Target:
	...
	def run(self):
		...
		mem_storage = cv2.CreateMemStorage(0)
		
		# The difference between the running average and the current frame:
		difference = cv2.CloneImage( display_image )
		...
		text_coord = ( 5, 15 )
		text_color = cv2.CV_RGB(255,255,255)

		###############################
		### Face detection stuff
		haar_cascade = cv2.Load( '/root/base/development/MASINT_Proposal_Work/motion_tracking/haarcascades/haarcascade_frontalface_alt.xml' )
		...
		while True:
			...
			camera_image = cv2.QueryFrame( self.capture )
			
			frame_count += 1
			frame_t0 = time.time()
			
			# Create an image with interactive feedback:
			display_image = cv2.CloneImage( camera_image )
			
			# Create a working "color image" to modify / blur
			color_image = cv2.CloneImage( display_image )
			...
			cv2.RunningAvg( color_image, running_average_image, 0.320, None )
			
			# Convert the scale of the moving average.
			cv2.ConvertScale( running_average_image, running_average_in_display_color_depth, 1.0, 0.0 )
			...
			cv2.Threshold( grey_image, grey_image, 2, 255, cv2.CV_THRESH_BINARY )
			# Smooth and threshold again to eliminate "sparkles"
			cv2.Smooth( grey_image, grey_image, cv2.CV_GAUSSIAN, 19, 0 )
			cv2.Threshold( grey_image, grey_image, 240, 255, cv2.CV_THRESH_BINARY )
			
			grey_image_as_array = numpy.asarray( cv2.GetMat( grey_image ) )
			non_black_coords_array = numpy.where( grey_image_as_array > 3 )
			# Convert from numpy.where()'s two separate lists to one list of (x, y) tuples:
			non_black_coords_array = list(zip( non_black_coords_array[1], non_black_coords_array[0] ))
			
			points = []   # Was using this to hold either pixel coords or polygon coords.
			bounding_box_list = []

			# Now calculate movements using the white pixels as "motion" data
			contour = cv2.FindContours( grey_image, mem_storage, cv2.CV_RETR_CCOMP, cv2.CV_CHAIN_APPROX_SIMPLE )
			
			while contour:
				
				bounding_rect = cv2.BoundingRect( list(contour) )
				point1 = ( bounding_rect[0], bounding_rect[1] )
				point2 = ( bounding_rect[0] + bounding_rect[2], bounding_rect[1] + bounding_rect[3] )
				
				bounding_box_list.append( ( point1, point2 ) )
				polygon_points = cv2.ApproxPoly( list(contour), mem_storage, cv2.CV_POLY_APPROX_DP )
				
				# To track polygon points only (instead of every pixel):
				#points += list(polygon_points)
				
				# Draw the contours:
				###cv2.DrawContours(color_image, contour, cv2.CV_RGB(255,0,0), cv2.CV_RGB(0,255,0), levels, 3, 0, (0,0) )
				cv2.FillPoly( grey_image, [ list(polygon_points), ], cv2.CV_RGB(255,255,255), 0, 0 )
				cv2.PolyLine( display_image, [ polygon_points, ], 0, cv2.CV_RGB(255,255,255), 1, 0, 0 )
				#cv2.Rectangle( display_image, point1, point2, cv2.CV_RGB(120,120,120), 1)
			...
			for box in bounding_box_list:
				cv2.Rectangle( display_image, box[0], box[1], cv2.CV_RGB(0,255,0), 1 )
			...
			if len(points):
				...
				for center_point in codebook:
					...
					#cv2.Circle(display_image, center_point, 10, cv2.CV_RGB(255, 0, 0), 2)
					#cv2.Circle(display_image, center_point, 5, cv2.CV_RGB(255, 0, 0), 3)
			...
			#	center_point = ( int(center_point[0]), int(center_point[1]) )
			#	cv2.Circle(display_image, center_point, 20, cv2.CV_RGB(255, 255,255), 1)
			#	cv2.Circle(display_image, center_point, 15, cv2.CV_RGB(100, 255, 255), 1)
			#	cv2.Circle(display_image, center_point, 10, cv2.CV_RGB(255, 255, 255), 2)
			#	cv2.Circle(display_image, center_point, 5, cv2.CV_RGB(100, 255, 255), 3)
			...
			for target in trimmed_center_points:
				...
				distance_list = list(entity_distance_dict.keys())
				distance_list.sort()
			...
			for entity in this_frame_entity_list:
				...
				cv2.Circle(display_image, center_point, 20, cv2.CV_RGB(c[0], c[1], c[2]), 1)
				cv2.Circle(display_image, center_point, 15, cv2.CV_RGB(c[0], c[1], c[2]), 1)
				cv2.Circle(display_image, center_point, 10, cv2.CV_RGB(c[0], c[1], c[2]), 2)
				cv2.Circle(display_image, center_point,  5, cv2.CV_RGB(c[0], c[1], c[2]), 3)
			...
			if image_name == "camera":
				...
				cv2.CvtColor( grey_image, display_image, cv2.CV_GRAY2RGB )
			...
			if self.writer: 
				cv2.WriteFrame( self.writer, image );
